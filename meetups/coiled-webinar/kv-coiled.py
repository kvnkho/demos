...
KV_KEY = "model_location"
BUCKET_NAME = "coiled-prefect"

MARKDOWN = """# {coin_name}

<img src=" https://{bucket}.s3.amazonaws.com/{image}">
"""
...
def detect_dip(df: pd.DataFrame, threshold, coin_name):
    ...
    my_result = S3Result(bucket=BUCKET_NAME, location="{flow_run_name}/" + coin_name + "/forecast.png", serializer=NoOpSerializer())
    res = my_result.write(buffer.getvalue(), **prefect.context)

    prefect.artifacts.create_markdown(MARKDOWN.format(coin_name=coin_name, bucket=BUCKET_NAME, image=res.location))
    
    return True

@task
def set_kv():
    flow_name = prefect.context.get("flow_run_name")
    set_key_value(key=KV_KEY, value={"bucket": BUCKET_NAME,"folder": flow_name, "time": datetime.now().strftime("%Y-%m-%d")})
